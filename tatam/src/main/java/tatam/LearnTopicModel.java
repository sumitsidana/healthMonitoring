...
public class LearnTopicModel {
	...
	public static void main(String[] args) throws Exception {
		...
		tatam.TopicModel topicModel = null;
		...
		if (model.equals("lda")) {
			...
			topicModel = new tatam.LDA(Z, alpha, beta, gamma0, gamma1);
		}
		else if (model.equals("atam")) {
			if (!arguments.containsKey("-Z")) {
				System.out.println("Must specify number of topics using -Z");
				return;
			}
			...
			if (!arguments.containsKey("-A")) {
				System.out.println("Must specify number of ailments using -A");
				return;
			}
			int A = Integer.parseInt(arguments.get("-A"));

			topicModel = new tatam.ATAMS(Z, 3, A, 0.1, 0.01, 0.01, 1.0, 1.0, 1.0, 1.0, 0.0, 0.01);
		}
		else {
