...
public class ATAMS extends TopicModel {
	...
	public HashMap<String,Integer> stampMap;
	public HashMap<Integer,String> stampMapInv;
	...
	public ATAMS(int z, int y, int ail, double a, double b, double w, double g0, double g1, 
			double d0, double d1, double lp,double m) {
		A = ail;
		Z = z;
		Y = y;
	...
	public void sampleD(int d) {
		...
		for (int a = 0; a < A; a++){
			double pa;
			pa = (nAT[a][t] + mu[a][t])/(nA[a]+muSum[a]);
			p[a] += Math.log(pa);
		}
	...
	public void readDocs(String filename) throws Exception {
		...
		stampMap = new HashMap<String,Integer>();
		stampMapInv = new HashMap<Integer,String>();
		...
		while ((s = br.readLine()) != null) {
			...
			docsC[d] = tokens0[0];
			String timeStamp = tokens0[1];
			int keyStamp = stampMap.size();
			if(!stampMap.containsKey(timeStamp)){
				stampMap.put(timeStamp, new Integer(keyStamp));
				stampMapInv.put(new Integer(keyStamp), timeStamp);
			}
			else{
				keyStamp = ((Integer)stampMap.get(timeStamp)).intValue();
			}
		...
		System.out.println(T+" TimeStamps");
	...
	public void writeOutput(String filename) throws Exception {
		...
		for (int d = 0; d < D; d++) {
			bw.write(docsC[d]+" "+docsA[d]+" "+stampMapInv.get(docsT[d])+" ");
