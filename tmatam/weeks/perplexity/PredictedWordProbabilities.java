...
public class PredictedWordProbabilities {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		Arrays.sort(chld);
		int numAilments = Integer.parseInt(args[4]);
		...
		for(int chldIndex = 0 ; chldIndex < chld.length ; chldIndex++){
			if(new File(args[0]+"/"+chld[chldIndex]).isDirectory()){
				...
				new File(args[7]+"/"+chld[chldIndex]).mkdirs();
				for(int chldBoundaryIndex = 0 ; chldBoundaryIndex
					...
					FileInputStream fstreamEntityCounts = new FileInputStream
							(args[1]+"/"+chld[chldIndex]+"/"+chldRegion[chldBoundaryIndex].
									replaceAll("_invertedindex", "_entitycount"));
					...
					FileInputStream fstreamEntityProb = new FileInputStream(args[6]+"/"+
							chld[chldIndex]+"/predicted_"+chldRegion[chldBoundaryIndex]+
							"_wordgivenentityprobability_" +
							"entitygivenwordprobability");
					...
					while((lineEntityProb = brEntityProb.readLine())!=null){
						...
						for(int i = 1 ; i < array.length ;i++){
							if(i <= numTopics+2){
								...
								int ind = i - numTopics-2;
								String index = ind + "";
								entityProb.put("Ailment "+index,Double.parseDouble(array[i]));								
							}
							...
							if(invertedIndex.containsKey(token)){
								...
								for(int j = 1 ; j <= numAilments ; j++ )
								{
									entityProbabilities.put("Ailment "+j, (double)1/
											(entityCounts.get("Ailment "+j)+vocab));
								}
					...
					PrintWriter out  = new PrintWriter(new BufferedWriter
							(new FileWriter(args[7]+"/"+chld[chldIndex]+"/"+
