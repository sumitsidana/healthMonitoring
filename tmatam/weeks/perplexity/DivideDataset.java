...
public class DivideDataset {
	 ...
	 *  that test data must contain last two weeks of the boundary.
	 *  If a 'timeline boundary' has less than four weeks - discard the boundary.
	 *   We obtain 581 boundaries for 349 regions as some boundaries do not have enough
	 *    tweets to test on.
	 */
			...
			for(int chldIndex = 0; chldIndex < chld.length ; chldIndex++){ 
				if(new File(args[0]+"/"+chld[chldIndex]).isDirectory()){
					...
					for(int i = 0 ; i < chldRegionalHealthFileList.length ; i++){
						...
						String line = null;
						Map<String,Map<String,String>>weeklyHealthTweets = new
						...
						while((line = brFile.readLine())!=null){
							...
							String text = line.substring(indexofFirstDelimiter+5, indexofSecondDelimiter);
							...
							int weekInt = dateTime.getWeekOfWeekyear();
							String week = ""+weekInt;
							if(weekInt<10)
								week = "0"+week;
							String year_week = year+"_"+week;
							Map<String,String>tweet = null;
							if(weeklyHealthTweets.containsKey(year_week)){
								tweet = weeklyHealthTweets.get(year_week);
							}
							else{
								tweet = new LinkedHashMap<String,String>();
							}
							tweet.put(tweetId, text);
							weeklyHealthTweets.put(year_week , tweet);
						}
						if(weeklyHealthTweets.size()<4)
						{
							continue;
						}
						...
						ArrayList<String> keys = new ArrayList<String>(weeklyHealthTweets.keySet());
						Map<String,Map<String,String>>train = new TreeMap<String,Map<String,String>>();
						Map<String,Map<String,String>>test = new TreeMap<String,Map<String,String>>();
						Set<String>weekTrack = new TreeSet<String>();
						for(int weekIndex=keys.size()-1; weekIndex>=0;weekIndex--){
							String year_week = keys.get(weekIndex);
							weekTrack.add(year_week);
							if(weekTrack.size()<=2){
								test.put(year_week, weeklyHealthTweets.get(keys.get(weekIndex)));
							}
							else
							{
								train.put(year_week, weeklyHealthTweets.get(keys.get(weekIndex)));
							}
						}
						...
						while(itTrain.hasNext()){
							...
							String year_week = (String) pairs.getKey();
