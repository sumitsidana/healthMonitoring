...
public class CreateATAMInputStopWordsUnleashedWithLowerCaseIssue {
    ...
    {
        createATAMInput.StanfordLemmatizer slem = new createATAMInput.StanfordLemmatizer();
    	...
    	try{
    		...
    		BufferedReader bufferedReaderSW = new BufferedReader(new InputStreamReader(dataInputStreamSW));
    		String strLineSW;
    		...
    		for(int i = 0 ; i < chld.length ; i++)
    		{
    			if(chld[i].equalsIgnoreCase("keywords_diseases.txt"))
    			{
    				...
    				FileInputStream fstream = new FileInputStream(args[1]+"keywords_diseases.txt");
    				...
    				while((strLine = br.readLine())!=null)
    				{
    					...
    					scrapedTokensDiseases.put(strLine.toLowerCase(), numberofHealthTokens++);
    				}
    				in.close();
    			}
    			...
    			{
    				System.out.println("processing file"+chld[i]);
    				FileInputStream fstream = new FileInputStream(args[1]+"keywords_symptoms.txt");
    				DataInputStream in = new DataInputStream(fstream);
    				BufferedReader br = new BufferedReader(new InputStreamReader(in));
    				String strLine;
    				while((strLine = br.readLine())!=null)
    				{
    					List<String>listofStrings = slem.lemmatize(strLine);
    					strLine = Concat(listofStrings);
    					scrapedTokensSymptoms.put(strLine.toLowerCase(), numberofHealthTokens++);
    				}
    				in.close();			
    			}
    			...
    			{
    				System.out.println("processing file"+chld[i]);
    				FileInputStream fstream = new FileInputStream(args[1]+"keywords_treatments.txt");
    				DataInputStream in = new DataInputStream(fstream);
    				BufferedReader br = new BufferedReader(new InputStreamReader(in));
    				String strLine;
    				while((strLine = br.readLine())!=null)
    				{
    					List<String>listofStrings = slem.lemmatize(strLine);
    					strLine = Concat(listofStrings);
    					scrapedTokensTreatments.put(strLine.toLowerCase(), numberofHealthTokens++);
    				}
    				in.close();
    			}
    		...
    		while((line = br.readLine())!=null)
    		{
    	        ...
    	        String tweetText = line.substring(indexoffirstTab+5);
    		...
    		{	
    	         ...
    	         if(scrapedTokensSymptoms.containsKey(newStringForComparison.toLowerCase()))
    	        	{
    	        		...
    	        		{
    	        			aspectofTokensOfTweet.put(spliterofSymptomsToken[j].toLowerCase(), 1);
    	        		}
    	        	}
 	        	
 	        	else if(scrapedTokensTreatments.containsKey(newStringForComparison.toLowerCase()))
 	        	{
 	        		...
 	        		{
 	        			aspectofTokensOfTweet.put(spliterofTreatmentsToken[j].toLowerCase(), 2);
 	        		}
 	        	}
    	         else if(scrapedTokensDiseases.containsKey(newStringForComparison.toLowerCase()))
    	        	{
    	        		String [] spliterofDiseasesToken = tokens[i].split
    	        				("[\\s+\\-*/\\^:;\\[\\]\\\\()_#@$%&|<>\'\",.?{}!=`~]+");
    	        		for(int j = 0 ; j < spliterofDiseasesToken.length ; j++)
    	        		{
    	        			if((aspectofTokensOfTweet.get(spliterofDiseasesToken[j].toLowerCase())!=1)||
    	        					(aspectofTokensOfTweet.get(spliterofDiseasesToken[j].toLowerCase())!=2))
    	        			aspectofTokensOfTweet.put(spliterofDiseasesToken[j].toLowerCase(), 0);
    	        		}
    	        	}
    		 ...
    		 PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter
    				 (args[4]+"/InputToATAMUsingLemmatizer"+args[3],true)));
    		while((line = br.readLine())!=null)
    		{	
    	        int indexoffirstTab = line.indexOf("##,##");
    	        String tweetText = line.substring(indexoffirstTab+5);
    	        out.write(line.substring(0, indexoffirstTab)+" ");
    	        String []tokens = tweetText.split("[\\s+\\-*/\\^:;\\[\\]\\\\()_#@$%&|<>\'\",.?{}!=`~]+");
    	        for(int i = 0 ; i < tokens.length ; i++)
    	        {
    	        	if(i < tokens.length - 1)
    	        	{
    	        		if(!(stopWords.contains(tokens[i].toLowerCase())))
    	        		out.write(tokens[i].toLowerCase()+":"+aspectofTokensOfTweet.get(tokens[i].toLowerCase())+" ");
    	        	}
    	        	else
    	        	{
    	        		if(!(stopWords.contains(tokens[i].toLowerCase())))
