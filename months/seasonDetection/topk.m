...
for i = 3:68
%     A =  dlmread(strcat('/media/TOSHIBA EXT_/regions/fullregion/training/',listing(i).name,'/ailments/trainfile'),',')
%     B =  dlmread(strcat('/media/TOSHIBA EXT_/regions/fullregion/training/',listing(i).name,'/ailments/testfile'),',')
...
%         datetime = strcat(year_week(1:index-1),'.',year_week(index+1:length(year_week)))
    ...
    hold on
     ...
     saveas(Figure,strcat('distributiondifferencessorted/',listing(i).name,'.png'))
     close all
     hold off
%     hold on
